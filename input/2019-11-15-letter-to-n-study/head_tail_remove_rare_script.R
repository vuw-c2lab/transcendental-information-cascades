...
links <- readr::read_csv("../../output/2019-11-15-letter-to-n-study/primes50k-random-discrete-tokenised-2019-11-21-20-42-39/createTIC/links.csv")
tokenProbs <- plyr::count(as.character(links$token))
tokenProbs$prob <- as.numeric(tokenProbs$freq)/nrow(links)


nodes <- readr::read_csv("../../output/2019-11-15-letter-to-n-study/primes50k-random-discrete-tokenised-2019-11-21-20-42-39/createTIC/nodes.csv")
setProbs <- plyr::count(nodes$tokens)
setProbs$prob <- as.numeric(setProbs$freq)/nrow(nodes)
distr <- plyr::count(setProbs$freq)
plot(distr,type='l')
...
for(i in 1:20){
  ...
  write_delim(as.data.frame(primes50k_tailRemove1),paste0("data/primes-random50k_tailRemove_",i,".txt"),delim = ";",col_names = F)
}
...
#     primes50k_tailRemove1[nodesToRem[j],3] <- paste0(tokens,collapse = ", ")
#   }
#   write_delim(as.data.frame(primes50k_tailRemove1),paste0("data/primes-random50k_tailRemove2_",i,".txt"),delim = ";",col_names = F)
# }
...
for(i in 1:20){
  ...
  write_delim(as.data.frame(primes50k_tailRemove1),paste0("data/primes-random50k_headRemove_",i,".txt"),delim = ";",col_names = F)
}
